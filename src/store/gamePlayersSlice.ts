...
export const gamePlayersSlice = createSlice({
		...
		reducers: {
			...
			'newPlayerDeals': (state: Draft<FourPlayerGameState>, action: PayloadAction<{ newId: string }>) => {
			...
			'replacePlayer': (state: Draft<FourPlayerGameState>, action: PayloadAction<{ playerToRemoveId: string, newPlayer: { id: string, name: string } }>) => {
				...
				if (newPlayer != undefined) {
					const indexToAddNewPlayer: GameStateKey | undefined = getIndexOfPlayer(state, playerToRemove.id);
					const indexToAddOldPlayer: GameStateKey | undefined = getIndexOfPlayer(state, newPlayer.id);
					...
					const playerToReplaceIndex: GameStateKey | undefined = getIndexOfPlayer(state, action.payload.playerToRemoveId);
...
function getPlayerById(state: FourPlayerGameState, playerId: string): GamePlayer | undefined {
	const players: readonly GamePlayer[] = getAllPlayers(state);
	return players.find(player => player.id === playerId);
}

function getIndexOfPlayer(state: FourPlayerGameState, playerId: string): GameStateKey | undefined {
...
export const selectGamePlayersInWeirdOrder = (state: { gamePlayers: FourPlayerGameState }): GamePlayer[] => {
	const players = state.gamePlayers;
	return [players['t1p1'], players['t2p1'], players['t1p2'], players['t2p2']];
}
export const selectPlayerWhoDeals = (state: { gamePlayers: FourPlayerGameState }): GamePlayer | undefined => {
